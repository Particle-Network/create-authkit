  ...
  try {
    ...
    if (!templateName) {
      const { value } = await prompts({
        ...
        name: 'value',
        choices: [
          { title: 'create-next-app', value: 'next-authkit-app' },
          { title: 'create-react-app', value: 'react-authkit-app' },
        ],
      });
      templateName = value;
    }
    ...
    log(chalk.cyan(`🚀 Creating a new Authkit app in ${chalk.bold(targetPath)}`));
    ...
    delete pkgJson.dependencies['@particle-network/authkit'];
    delete pkgJson.dependencies['@particle-network/wallet'];
    ...
    if (!options.skipGit) {
      ...
      await execa('git', ['commit', '--no-verify', '--message', 'Initial commit from create-authkit'], {
        cwd: targetPath,
      });
    }

    log(chalk.green('🤩 Done! Thanks for using Particle Network 🙏'));
    log();
    log(
      chalk.cyan(
        `👉 To get started, run ${chalk.bold(`cd ${projectPath}`)} and then ${chalk.bold(
          `${packageManager}${packageManager === 'npm' ? ' run' : ''} ${
            templateName.includes('next') ? 'dev' : 'start'
          }`
        )}`
      )
    );
