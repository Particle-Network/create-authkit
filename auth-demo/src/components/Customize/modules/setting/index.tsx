            ...
            {[formData.lightGroups, formData.darkGroups].map((themeGroup, themeIndex) => {
              return (
                <Collapse
                  ...
                  style={{
                    display:
                      (themeMode === 'lightMode' && themeIndex === 0) || (themeMode === 'darkMode' && themeIndex === 1)
                        ? 'block'
                        : 'none',
                  }}
                >
                              ...
                              {data.type === ItemType.Number && (
                                <Form.Item name={data.key} initialValue={data.value}>
                                  <InputNumber
                                    ...
                                    onChange={(e) => {
                                      const value = form.getFieldValue(data.key);
                                      changeHandler(data.key, Number(value as any) || 0);
                                    }}
                                  />
                                </Form.Item>
                              )}
                ...
                <div
                  ...
                  onClick={() => {
                    if (
                      ...
                      message.error('Please wait for the iframe to load before loading the theme');
                      return false;
                    }
                    ...
                    Object.keys(formData.lightMode).forEach((key) => {
                      const value = currentCustomStyle?.[key] || currentCustomStyle?.theme?.light?.[key];
                      form.setFieldValue(
                        ['lightMode', key],
                        currentCustomStyle?.[key] ||
